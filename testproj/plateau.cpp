...
void Plateau::distribuer() {
    ...
    unsigned nbCardTemp = unsigned(this->indexDeck.size());
    unsigned nbP = this->getNbPlayer();
    std::cout << "NbTotal de cartes à Distribuer : " << nbCardTemp <<std::endl;
    for(unsigned i=0; i<nbCardTemp; i++)
    {
        this->m_players[i%nbP].addCard(this->indexDeck[i]);
    }
}
...
void Plateau::RemoveCard(unsigned int playerIndex, unsigned int cardIndex){
    int card = this->m_players[playerIndex].getCardByIndex(cardIndex);
    //WIRE wire = this->m_cards[unsigned(this->m_players[playerIndex].getCardByIndex(cardIndex))].getType();
    if( this->m_cards[unsigned(this->m_players[playerIndex].getCardByIndex(cardIndex))].getType() == BOMB)
        this->bomb = true;
    else if( this->m_cards[unsigned(this->m_players[playerIndex].getCardByIndex(cardIndex))].getType() == DEFUSING)
        this->nbWiresATrouver--;
    
    this->nbCableACouper--;
    this->m_players[playerIndex].removeCardByIndex(cardIndex);
    bool r = false;
    for(unsigned i = 0; i < this->indexDeck.size() && !r; i++){
        if(this->indexDeck[i] == card ){
            this->indexDeck[i] = -1;
            r = true;
        }
    }
...
void Plateau::startGame() {
    ...
    do {
        std::cout << "nbCable a couper:";
        std::cout << this->nbCableACouper << std::endl;
        std::cout << "nbCable a TROUVER:";
        std::cout << this->nbWiresATrouver << std::endl;
        std::cout << "nbPlayer:";
